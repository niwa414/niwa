...
def load_variants(path: Path) -> list[str]:
    ...
    ordered = []
    for key in ("control", "floor_only", "substeps_only", "treatment"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run B4 hard sweep variants.")
    parser.add_argument(
        "--variants",
        required=True,
        help="Path to variants.json describing control/treatment cases.",
    )
    parser.add_argument(
        "--stage",
        default="all",
    ...
    for case_id in case_ids:
        if args.skip_existing and should_skip(case_id):
            print(f"[b4-hard] skip existing PASSFAIL: {case_id}")
            continue
        cmd = [
            sys.executable,
            "tools/run_case.py",
        ...
        print(f"[b4-hard] running: {' '.join(cmd)}")
