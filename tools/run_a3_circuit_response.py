...
def load_variants(path: Path) -> list[str]:
    ...
    ordered = []
    for key in ("base", "baseline", "control", "ampplus10", "perturb", "treatment"):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run A3 circuit response variants (base vs perturb)."
    )
    parser.add_argument(
        "--variants",
        required=True,
        help="Path to variants.json describing base/perturb cases.",
    )
    parser.add_argument(
        "--stage",
        default="all",
    ...
    for case_id in case_ids:
        if args.skip_existing and should_skip(case_id):
            print(f"[a3] skip existing PASSFAIL: {case_id}")
            continue
        cmd = [
            sys.executable,
            "tools/run_case.py",
